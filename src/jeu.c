...
{   
    ...
    if(ptr_rcp_recup->pptr_btn_board[int_y][int_x].int_state == 1){
        printf("roejgerioj");
        return 0;
    }
    ...
    printf("pass");
    if(keycode == 1)
        return (reveal(ptr_rcp_recup, int_x, int_y));
...
int reveal(t_recup* ptr_rcp_recup, int int_x, int int_y)
{
    ...
    if( ptr_rcp_recup->pptr_btn_board[int_y][int_x].int_neighbor == 0) {
        ...
        for( i = (int_x == 0) ? 0 : -1; i < int_R_edge; i++)
            for(j = (int_y == 0) ? 0 : -1; i < int_B_edge; i++)
